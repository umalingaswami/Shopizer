---
title: Dropping and Integrating Form Items
---
This document describes how users can drag and drop items onto a form or canvas, supporting both adding new items and moving existing ones. The flow validates, transforms, and integrates items, replaces existing items if necessary, and finalizes the drop by selecting the item for editing.
...
flowchart TD
  ...
  node10 -->|"No"| node11["Load item asynchronously"]
  ...
  node11 --> node12["Complete item drop and finalize"]
  click node12 openCode "shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js:2052:2057"
  node10 -->|"Yes"| node12
  node9 --> node12
...
%%   node6 --> node7{"Was transformation successful?"}
%%   click node7 openCode "<SwmPath>[shopizer/…/modules/ISC_DataBinding.js](shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js)</SwmPath>:2046:2047"
%%   node7 -->|"No"| node8["Abort drop"]
...
%%   node11 --> node12["Complete item drop and finalize"]
%%   click node12 openCode "<SwmPath>[shopizer/…/modules/ISC_DataBinding.js](shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js)</SwmPath>:2052:2057"
%%   node10 -->|"Yes"| node12
%%   node9 --> node12
%% 
...
This section governs how users can drag and drop form items onto a form or canvas, ensuring that only valid items are added, that items are transformed and wrapped as needed, and that asynchronous data loading and custom business logic are respected during the drop process.

| Category        | Rule Name                      | Description                                                                                                                                                             |
| --------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Data validation | Valid Drag Data Required       | Only valid drag data can be used to create or move a form item. If the drag data is missing, the drag target is used instead. If neither is valid, the drop is aborted. |
| Data validation | Asynchronous Data Loading      | If the dropped item requires asynchronous data loading, the drop is not finalized until all required data is loaded.                                                    |
| Data validation | Single Child Enforcement       | If the parent container does not allow multiple children of the dropped item type, any existing child of that type is removed before adding the new item.               |
| Business logic  | Non-Palette Item Move          | If the drag source is not a Palette, the item is prepared for moving by removing it from its parent and adjusting its position in the form.                             |
| Business logic  | Custom Drop Logic              | Before an item is dropped, custom logic may modify or veto the drop. If vetoed, the drop is aborted and the item is not added.                                          |
| Business logic  | Item Wrapping and Data Binding | Dropped items must be wrapped in the correct container and bound to the appropriate data source before being added to the form.                                         |
| Business logic  | Auto-Select Dropped Item       | After a successful drop, the new item is automatically selected for editing, allowing the user to configure it immediately.                                             |

<SwmSnippet path="/shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" line="2045">

---

In <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2045:5:5" line-data=",isc.A.itemDrop=function isc_DynamicForm_itemDrop(_1,_2,_3,_4,_5,_6){var _7=_1.getDragData();if(_7==null){_7=isc.EH.dragTarget;if(isc.isA.FormItemProxyCanvas(_7)){this.logInfo(&quot;The dragTarget is a FormItemProxyCanvas for &quot;+_7.formItem,&quot;editModeDragTarget&quot;);_7=_7.formItem}}">`itemDrop`</SwmToken>, we start by grabbing the drag data from the drag source. If that's missing, we fall back to the drag target from the event handler, and if it's a <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2045:55:55" line-data=",isc.A.itemDrop=function isc_DynamicForm_itemDrop(_1,_2,_3,_4,_5,_6){var _7=_1.getDragData();if(_7==null){_7=isc.EH.dragTarget;if(isc.isA.FormItemProxyCanvas(_7)){this.logInfo(&quot;The dragTarget is a FormItemProxyCanvas for &quot;+_7.formItem,&quot;editModeDragTarget&quot;);_7=_7.formItem}}">`FormItemProxyCanvas`</SwmToken>, we switch to the real form item. This lets us handle both direct and proxy drag targets. If the drag source isn't a Palette, we prep for moving the component in the edit context by removing it from its parent and adjusting the drop index if needed. We call <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2046:84:84" line-data="if(!_1.isA(&quot;Palette&quot;)){if(isc.EditContext.$70r)isc.EditContext.$70r.hide();var _8=this.editContext.data,_9=_8.getParent(_7.editNode),_10=_8.getChildren(_9).indexOf(_7.editNode),_11=_7.editNode;if(isc.isA.Function(this.itemDropping)){_11=this.itemDropping(_11,_2,true);if(!_11)return}">`itemDropping`</SwmToken> next to let any custom logic modify or veto the drop before we actually add the node.
...
Back in <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2045:5:5" line-data=",isc.A.itemDrop=function isc_DynamicForm_itemDrop(_1,_2,_3,_4,_5,_6){var _7=_1.getDragData();if(_7==null){_7=isc.EH.dragTarget;if(isc.isA.FormItemProxyCanvas(_7)){this.logInfo(&quot;The dragTarget is a FormItemProxyCanvas for &quot;+_7.formItem,&quot;editModeDragTarget&quot;);_7=_7.formItem}}">`itemDrop`</SwmToken>, after <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2046:84:84" line-data="if(!_1.isA(&quot;Palette&quot;)){if(isc.EditContext.$70r)isc.EditContext.$70r.hide();var _8=this.editContext.data,_9=_8.getParent(_7.editNode),_10=_8.getChildren(_9).indexOf(_7.editNode),_11=_7.editNode;if(isc.isA.Function(this.itemDropping)){_11=this.itemDropping(_11,_2,true);if(!_11)return}">`itemDropping`</SwmToken> returns, we remove the old node from its parent and use <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2047:31:31" line-data="this.editContext.removeComponent(_11);if(_9==this.editNode&amp;&amp;_2&gt;_10)_2--;var _12=this.editContext.addNode(_7.editNode,this.editNode,_2);if(_12&amp;&amp;_12.liveObject){isc.EditContext.delayCall(&quot;selectCanvasOrFormItem&quot;,[_12.liveObject,true],200)}">`addNode`</SwmToken> to insert it at the new position in the edit context. This updates the component tree and UI to reflect the drop. We also select the new item for editing.
...
<SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2172:21:21" line-data=");isc.B._maxIndex=isc.C+16;isc.EditContext.addInterfaceMethods({addNode:function(_1,_2,_3,_4,_5){var _6=this.getEditNodeTree();if(_2==null)_2=this.getDefaultParent(_1);var _7=this.getLiveObject(_2);this.logInfo(&quot;addComponent will add newNode of type: &quot;+_1.type+&quot; to: &quot;+this.echoLeaf(_7),&quot;editing&quot;);if(_7.wrapChildNode){_2=_7.wrapChildNode(this,_1,_2,_3);if(!_2)return;_7=this.getLiveObject(_2)}">`addNode`</SwmToken> figures out where to insert the new node, checks if the parent allows multiple children of that type, and if not, removes the old one. It then adds the node to the edit tree, sets up the live object, and triggers any hooks for node addition. If the node needs to load data asynchronously, that's handled separately.
...
After <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2047:31:31" line-data="this.editContext.removeComponent(_11);if(_9==this.editNode&amp;&amp;_2&gt;_10)_2--;var _12=this.editContext.addNode(_7.editNode,this.editNode,_2);if(_12&amp;&amp;_12.liveObject){isc.EditContext.delayCall(&quot;selectCanvasOrFormItem&quot;,[_12.liveObject,true],200)}">`addNode`</SwmToken> in <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2045:5:5" line-data=",isc.A.itemDrop=function isc_DynamicForm_itemDrop(_1,_2,_3,_4,_5,_6){var _7=_1.getDragData();if(_7==null){_7=isc.EH.dragTarget;if(isc.isA.FormItemProxyCanvas(_7)){this.logInfo(&quot;The dragTarget is a FormItemProxyCanvas for &quot;+_7.formItem,&quot;editModeDragTarget&quot;);_7=_7.formItem}}">`itemDrop`</SwmToken>, if the drag source is a Palette, we transfer the drag data and check if it needs to be loaded. If so, we load it asynchronously and only call <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2049:8:8" line-data="_15.isLoaded=true;_14.completeItemDrop(_15,_2,_3,_4,_5,_6)">`completeItemDrop`</SwmToken> once that's done, so the drop is finalized with all data ready. Otherwise, we call <SwmToken path="shopizer/sm-shop/src/main/webapp/resources/smart-client/system/modules/ISC_DataBinding.js" pos="2049:8:8" line-data="_15.isLoaded=true;_14.completeItemDrop(_15,_2,_3,_4,_5,_6)">`completeItemDrop`</SwmToken> right away.
